...
// NewDefaultClient returns Client with logging tripperware.
...
func (c *Client) RulesInGRPC(ctx context.Context, base *url.URL, typeRules string) ([]*RuleGroup, error) {
	u := *base
	...
	var m struct {
		Data RuleAPI `json:"data"`
	}
	return m.Data.RuleGroups, c.get2xxResultWithGRPCErrors(ctx, "/prom_rules HTTP[client]", &u, &m)
}
