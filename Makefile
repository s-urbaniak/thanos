...
# test-infra https://github.com/prometheus/test-infra/pull/325
FUNCBENCH_VERSION       ?= be550a51c589b830cf146e256c4bfdf6069d49c7
...
.PHONY: bench
bench: ## Run all benchmarks and compare with master using https://github.com/prometheus/test-infra/tree/master/funcbench.
bench: $(FUNCBENCH)
	@echo ">> benchmark and compare with master"
	@$(FUNCBENCH) -v --bench-time=30s --timeout=2h --result-cache=/tmp/cache master "*"

.PHONY: bench-master
bench-master: ## Run all benchmarks and compare with newest using https://github.com/prometheus/test-infra/tree/master/funcbench.
bench-master: $(FUNCBENCH)
	@echo ">> benchmark and compare with: $(shell git tag | grep -E ^v[0-9]+\.[0-9]\.[0-9]+$$ | sort -t . -k1,1n -k2,2n -k3,3n | tail -1)"
	@$(FUNCBENCH) -v --bench-time=30s --timeout=2h --result-cache=/tmp/cache  "$(shell git tag | grep -E ^v[0-9]+\.[0-9]\.[0-9]+$$ | sort -t . -k1,1n -k2,2n -k3,3n | tail -1)" "*"

.PHONY: bench-release
bench-release: ## Run all benchmarks and compare with older release using https://github.com/prometheus/test-infra/tree/master/funcbench.
bench-release: $(FUNCBENCH)
	@echo ">> benchmark and compare with"
	@$(FUNCBENCH) -v --bench-time=30s --timeout=2h --result-cache=/tmp/cache "$(shell git tag | grep -E ^v[0-9]+\.[0-9]\.[0-9]+$$ | sort -t . -k1,1n -k2,2n -k3,3n | grep -B1 $(CURRENT_RELEASE) | head -1)" "*"
